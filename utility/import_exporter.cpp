...
Error ImportExporter::remap_resource(const String &output_dir, Ref<ImportInfo> &iinfo) {
	...
	Error err;
	Ref<FileAccess> f = FileAccess::open(output_dir.plus_file(iinfo->source_file.replace("res://", "")) + ".remap", FileAccess::WRITE, &err);
	ERR_FAIL_COND_V(err, err);
...
Error ImportExporter::rename_dependency_in_resource(const Ref<ImportInfo> p_iinfo, const HashMap<String, String> &p_rename_map) {
	...
	if (err == ERR_FILE_UNRECOGNIZED) {
		...
		for (auto E : p_rename_map) {
			String source = E.key.replace("res://", "");
			String new_source = E.value.replace("res://", "");
			txt.replace(source, new_source);
		}
		Ref<FileAccess> f = FileAccess::open(p_iinfo->import_path, FileAccess::WRITE, &err);
...
Error ImportExporter::rename_imports(const String &output_dir) {
	...
	RBMap<String, Ref<ImportInfo>> iinfos;
	Error err;
	for (int i = 0; i < files.size(); i++) {
		Ref<ImportInfo> iinfo = files[i];
		String path = iinfo->get_path();
	...
	for (auto E : rename_map) {
		String source = E.key;
		String new_source = E.value;
	...
	for (int i = 0; i < gd_files.size(); i++) {
		...
		Ref<FileAccess> f = FileAccess::open(gd_files[i], FileAccess::WRITE, &err);
		if (!err) {
			f->store_string(txt);
		}
	}
